...
class tx_realty_openImmoImport {
	...
	public function importFromZip() {
		...
		if (!empty($zipsToExtract)) {
			...
			$this->addToErrorLog(
				$this->getTranslator()->translate('message_no_zips')
			);
		}
	...
	protected function writeToDatabase(array $realtyRecord, $overridePid = 0) {
		...
		case 'message_deleted_flag_causes_deletion':
			...
			// error log.
			$this->addToLogEntry(
				$this->getTranslator()->translate($errorMessage) . LF
			);
			break;
	...
	public function extractZip($zipToExtract) {
		...
		if ($zip->open($zipToExtract)) {
			...
			if ($extractionDirectory != '') {
				...
				$this->addToLogEntry(
					$zipToExtract . ': ' . $this->getTranslator()->translate(
	...
	public function createExtractionFolder($pathOfZip) {
		...
		if (!is_dir($folderForZipExtraction)) {
			...
			$this->addToErrorLog(
				$folderForZipExtraction . ': ' .
					$this->getTranslator()->translate('message_surplus_folder')
			);
			$folderForZipExtraction = '';
		}
	...
	public function cleanUp($importDirectory) {
		...
		if (!empty($removedFiles)) {
			$this->addToLogEntry(
				$this->getTranslator()->translate('message_files_removed') .
...
if (defined('TYPO3_MODE') && $GLOBALS['TYPO3_CONF_VARS'][TYPO3_MODE]['XCLASS']['ext/realty/lib/class.tx_realty_openImmoImport.php']) {
	include_once($GLOBALS['TYPO3_CONF_VARS'][TYPO3_MODE]['XCLASS']['ext/realty/lib/class.tx_realty_openImmoImport.php']);
}
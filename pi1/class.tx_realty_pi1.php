...
class tx_realty_pi1 extends tx_oelib_templatehelper {
	...
	private function getHtmlForCurrentView() {
		...
		switch ($this->getCurrentView()) {
			...
			case 'single_view':
				...
				break;
			case 'contact_form':
				$contactForm = t3lib_div::makeInstance(
					'tx_realty_contactForm', $this->conf, $this->cObj
				);
				$formData = $this->piVars;
				$favoritesList = t3lib_div::makeInstance(
	...
	private function checkAccessAndGetHtmlOfErrorView() {
		...
		try {
			...
			$errorView = t3lib_div::makeInstance(
...
if (defined('TYPO3_MODE') && $GLOBALS['TYPO3_CONF_VARS'][TYPO3_MODE]['XCLASS']['ext/realty/pi1/class.tx_realty_pi1.php']) {
	include_once($GLOBALS['TYPO3_CONF_VARS'][TYPO3_MODE]['XCLASS']['ext/realty/pi1/class.tx_realty_pi1.php']);
}